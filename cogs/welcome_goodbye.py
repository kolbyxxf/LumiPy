...
class WelcomeGoodbye(commands.Cog):
    ...
    async def set_welcome(self, interaction: discord.Interaction, channel: discord.TextChannel):
        await storage.set_welcome_channel(str(interaction.guild.id), channel.id)
    ...
    async def set_goodbye(self, interaction: discord.Interaction, channel: discord.TextChannel):
        await storage.set_goodbye_channel(str(interaction.guild.id), channel.id)
        await interaction.response.send_message(f"Goodbye channel set to {channel.mention}.", ephemeral=True)

    @commands.Cog.listener()
    async def on_member_join(self, member: discord.Member):
        channel_id = await storage.get_welcome_channel(str(member.guild.id))
        if channel_id:
    ...
    async def on_member_remove(self, member: discord.Member):
        channel_id = await storage.get_goodbye_channel(str(member.guild.id))
        if channel_id:
            channel = member.guild.get_channel(channel_id)
            if channel: await channel.send(f"Goodbye {member.display_name}, we'll miss you! 👋")
